...
from configuration_server.shell_commands import get_oprp_pid, is_port_used_by_another_process, NoResponseException, \
...
class TestShellCommands:
    ...
    @patch('subprocess.Popen.communicate')
    def test_if_possible_to_separate_between_test_instance_and_other_process(self, popen_communicate_mock):
        port = 9001
        ...
        self._start_http_server_thread(9000)
        assert is_port_used_by_another_process(9000)

    def _allocate_next_free_port(self):
        port_db = PortDatabase(is_port_used_func=is_port_used_by_another_process)
        return port_db.allocate_port("issuer", "instance_id", PortDatabase.STATIC_PORT_TYPE, 8000, 8100)
